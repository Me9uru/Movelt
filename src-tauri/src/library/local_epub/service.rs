...
use crate::domain::{ChapterContent, ChapterSummary, NovelDetail, NovelOverview, Volume};
...
impl LocalEpubSource {
    ...
    pub(crate) fn new(root: PathBuf) -> Result<Self, NovelError> {
        fs::create_dir_all(&root).map_err(NovelError::epub_io)?;
        Ok(Self { root })
    }
    ...
    pub(crate) fn import(&self, source_path: &str) -> Result<NovelOverview, NovelError> {
        ...
        input.read_to_end(&mut data).map_err(NovelError::epub_io)?;
        hasher.update(&data);
        ...
        fs::create_dir_all(staging.join("content")).map_err(NovelError::epub_io)?;
        let result = (|| {
            fs::write(staging.join("book.epub"), &data).map_err(NovelError::epub_io)?;
            extract_archive(Cursor::new(&data), &staging.join("content"))?;
            self.overview_at(
                &id,
                &staging.join("content"),
                path.file_stem().and_then(|x| x.to_str()),
            )?;
            fs::rename(&staging, &book_dir).map_err(NovelError::epub_io)?;
            self.overview(&id)
        })();
        if result.is_err() {
            let _ = fs::remove_dir_all(&staging);
    ...
    pub(crate) fn remove(&self, book_id: &str) -> Result<(), NovelError> {
        let path = self
            ...
            .ok_or(NovelError::NotFound)?
            .to_path_buf();
        fs::remove_dir_all(path).map_err(NovelError::epub_io)
    }
...
fn extract_archive<R: Read + Seek>(reader: R, output: &Path) -> Result<(), NovelError> {
    ...
    for index in 0..archive.len() {
        ...
        if let Some(parent) = target.parent() {
            fs::create_dir_all(parent).map_err(NovelError::epub_io)?;
        }
        let mut file = File::create(target).map_err(NovelError::epub_io)?;
        std::io::copy(&mut entry, &mut file).map_err(NovelError::epub_io)?;
    }
    Ok(())
}
