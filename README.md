...
基于 Tauri 2、Vue 3 与 Rust 构建，以 Material Design 3 界面提供小说与漫画浏览、书架、阅读、账号与进度同步能力。
...
Movel 是 [LightNovel 轻书架](https://www.lightnovel.life/) 的非官方第三方客户端。前端采用 Material Design 3，并针对桌面窗口与移动设备提供自适应的浏览、详情和阅读界面；Rust 后端作为官方 API 的客户端边界，负责认证、会话续期、请求与响应校验及领域 DTO 映射；两端通过 Tauri IPC 通信。
...
![Movel 漫画发现页：自适应导航、推荐分栏与作品网格](docs/images/novels-overview.png)

## 功能

- **小说与漫画发现**：通过推荐、排行榜和搜索分栏浏览作品，支持按书名、作者或标签搜索与结果分页；首页集中展示最近更新和热门作品。
- **作品详情与目录**：在自适应详情页查看封面、作者、状态、简介、标签、最新章节和分卷目录，并可继续阅读或管理书架收藏。
...
- **Material Design 3 界面**：统一的色彩、层级、圆角与交互状态；宽屏使用侧边导航轨，窄屏切换为底部导航，设置通过头像抽屉随时打开。
- **响应式与移动端行为**：作品网格、详情页和阅读器随窗口宽度调整，并处理安全区域与 Android 返回导航。
...
| 自适应 MD3 界面 | 桌面侧边导航、移动端底部导航、响应式作品网格与统一的设置抽屉。 |
