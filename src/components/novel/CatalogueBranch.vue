...
import type { ChapterSummary, Volume } from "../../domain/content";
...
<template>
  <var-collapse v-if="volume.sections.length" v-model="activeSections" class="catalogue catalogue-nested">
    <var-collapse-item v-for="(section, index) in volume.sections" :key="`${section.title}-${index}`"
      ...
      <template #title>
        <div class="volume-title volume-title--section">
          ...
          <var-chip class="count-tag" size="small">{{ countChapters(section) }} 话</var-chip>
        </div>
      </template>
  ...
  <div v-if="volume.chapters.length > chaptersPerPage" class="chapter-pagination">
    ...
    <var-pagination :max-pager-count="5" :size="chaptersPerPage"
      :total="volume.chapters.length" :current="page" @update:current="page = Number($event)" />
  </div>
</template>
